    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHTML = details.participants.length > 0
          ? `<ul class="participants-list">${details.participants.map(p => `<li><span class="participant-email">${p}</span><button class="remove-btn" title="Unregister" data-activity="${name}" data-email="${p}">&times;</button></li>`).join("")}</ul>`
